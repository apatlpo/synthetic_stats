...
_default_tau_bounds = [0.0, 100.0]  # days
_default_omega_bounds = [0.01 * rad, 6 * rad]  # rpd
#
_default_low = dict(T=T, U=U)
_default_low_values = dict(T=10, U=1.0)
#
_default_high = dict(T=T, U=U, Us=Us, sigma=sigma)
_default_high_values = dict(T=10, U=1.0, Us=1.0, sigma=2.0 * rad)
...
def _xr_tau(
    ...
    if bounds is None:
        bounds = _default_tau_bounds
    tau = np.linspace(bounds[0], bounds[1], N)
    da = xr.DataArray(tau, dims=["tau"], coords={"tau": (["tau"], tau)}, name="tau")
    return da
...
class signal(object):
    ...
    def update_autocorrelation(self):
        ...
        self.autocorrelation = (
            inverse_fourier_transform(u.spectrum, omega, tau / 2 / pi) / 2 / pi
        )
    ...
    def update_variance(self):
        self.variance = (self.spectrum.integrate((omega, -oo, oo)) / 2 / pi).simplify()

    def update_parameters(self):
        self.p = {
            str(s): s for s in self.autocorrelation.free_symbols if str(s) != "tau"
        }

    def update_lambdas(self):
        _p = list(self.p.values())
        self.autocorrelation_lbd = lambdify([tau, *_p], self.autocorrelation, "numpy")
        if hasattr(self, "stationary") and self.stationary:
            ...
            modules = "numpy"
        self.spectrum_lbd = lambdify([omega, *_p], self.spectrum, modules)
    ...
    def evaluate_autocorrelation(
        ...
        **parameters_values,
        ...
        da = (
            self.autocorrelation_lbd(tau, *p)
            .rename(name)
    ...
    def evaluate_spectrum(
        ...
        **parameters_values,
        ...
        p_values.update(**parameters_values)
        p = [p_values[key] for key in self.p]
        da = self.spectrum_lbd(omega, *p).rename(name).assign_attrs(**parameters_values)
...
_analytical_spectrum_high = {
    ...
    "stationary": (
        Us**2 / 2 * pi * (DiracDelta((sigma - omega)) + DiracDelta((sigma + omega)))
    ),
}
...
class high_frequency_signal(signal):
    ...
    def __init__(
        ...
        if not parameters_values:
            ...
            p_values = parameters_values
            if "Us" in p_values and p_values["Us"] != 0.0:
                stationary = True
        #
        # bypass symbolic computation of the spectrum
