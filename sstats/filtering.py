...
def analytical_variance_slow(Td, T, omega, std=1.):
...
def analytical_variance_stationary(omega_0, T, omega, std=1.):
    """ Predicts the variance of a filtered signal according to:
    \int_{t-T/2}^{t+T/2} u(t) e^{-i\omega t} dt
...
def analytical_variance_nonstationary(Td, omega_0, T, omega, std=1.):
    """ Predicts the variance of a filtered signal according to:
    \int_{t-T/2}^{t+T/2} u(t) e^{-i\omega t} dt
    ...
    return std**2 * (Ip0+Ip1)/4.

def _analytical_It(Td, omega):
    """ Core integral used to compute analytical variances
    """
    return (2*Td*(Td**3*omega**2 + Td**2*omega**2
                 + Td*(-Td**2*omega**2*np.cos(omega)
...
def _analytical_Ic(omega_0, omega):
    ...
    #else:
    return (4*((omega*np.sin((1/2)*omega)*np.cos((1/2)*omega_0) 
